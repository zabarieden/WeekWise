...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const breakdownNote = `Before calling the tool, think step by step in plain text (not inside the tool call): list every distinct food item in the description separately (treating a slash-separated set of alternatives, per the note above, as ONE item, not several), and for each one write your best calorie estimate AND your best protein-in-grams estimate for the stated (or inferred, per the notes above) quantity. Then explicitly add together the numbers for these distinct items and state both sums. Only after that, call the tool with the summed calories as "calories" and the summed protein as "protein_grams" (use 0, not null, for items with negligible protein). Do not skip straight to a single holistic guess for the whole meal - a multi-item meal estimated item-by-item and then summed is measurably more accurate than one intuitive total, and this step is required even when it feels obvious.`;
        // הנחיה חדשה חמישית: "/" (סלאש) בין שני תיאורי-מזון הוא "או" (חלופה
        // לא-ודאית), לא "וגם" - לפי בקשה מפורשת אחרי שהתגלה בפועל:
        // "קופסאת טונה בשמן / חזה עוף 130 גרם" (המשתמשת אכלה אחד מהשניים,
        // לא בטוחה איזה, ורושמת את זה כ"או-או") חושב כשני פריטים נפרדים
        // ומצטרף אליהם יחד בסכום (כאילו אכלה את שניהם) במקום שיינתן ממוצע
        // בין שתי האפשרויות עבור אותו פריט יחיד. חייב להופיע בפרומפט *לפני*
        // breakdownNote כדי שהפירוק-לפריטים כבר יתייחס לחלופות-סלאש כפריט
        // אחד ולא כשניים
        const slashAlternativesNote = `A "/" (slash) placed between two or more food descriptions (not inside a quantity fraction like "2/3 cup" or "1/2") means the user is unsure exactly which of those options they ate and is logging it as an either/or - for example "a can of tuna in oil / chicken breast 130g" means they ate ONE of the two, not both. Treat everything on both sides of such a slash as alternative descriptions of the SAME single food item, not as separate items to add together. Estimate each alternative on its own, then use the AVERAGE of their calorie and protein values as your single result for that item - do NOT sum the alternatives as if the user ate all of them. This only applies to a slash used this way between distinct food descriptions; do not apply it to a slash that's just part of a quantity or fraction.`;
        // הנחיה חדשה רביעית: כשהתשובה עצמה מכילה מספר קלוריות מפורש (בין אם
        // זו תשובה לשאלת-הבהרה של ה-AI, ובעיקר בין אם זו תיקון-יזום של
        // המשתמשת אחרי "לא נראה לי נכון" - ר' rejectFoodEstimate ב-app.js) -
        // המספר הזה הוא עובדה מהמשתמשת (שיודעת מה היא באמת אוכלת/מדדה), לא
        // עוד רמז לשקלל מול ההערכה העצמאית של ה-AI. בלי ההנחיה הזו ה-AI עלול
