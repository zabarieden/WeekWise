...
// תקרת זמן קשיחה - הועלתה מ-60 ל-90 שניות (לפי בקשה מפורשת), אחרי שהתגלה
// בפועל דרך Network tab שקריאות עם הרבה רכיבים + חיפוש-רשת + חישוב-פירוק
// מפורש (ר' breakdownNote ב-estimate-food-text) נחתכות ב-60 שניות בדיוק
// ונופלות בשקט לחישוב המקומי בלי שום הודעה - כך שהמשתמשת חשבה שהיא מקבלת
// הערכת AI מדויקת בזמן שבפועל תמיד קיבלה את הגיבוי הגס. AbortController
// מבטל את הבקשה בפועל (לא רק מפסיק לחכות לה בצד שלנו) כדי לא להשאיר קריאה
// תקועה ברקע
const FOOD_AI_TIMEOUT_MS = 90000;
...
// תשלום (יש לו מסגרת - סכום כולל קבוע שידוע מראש) לעומת הוראת קבע (בלי
// מסגרת - חיוב פתוח שחוזר בלי סכום כולל ידוע) הוא עכשיו בחירה מפורשת עם
// צ'קבוקס, לא תוצר-לוואי של מילוי שדה המחשבון - לפי בקשה מפורשת אחרי
// שפריטים שנוספו מ"הוראות קבע" נחתו בטעות ב"תשלומים" כי המחשבון היה גלוי
// תמיד. כשזה תשלום - תאריך הסיום מחושב אוטומטית מתאריך ההתחלה + מספר
...
const FOOD_CALORIE_DB = [
    ...
    // עוגיית עבאדי שוקלת כ-12.5 גרם ליחידה בפועל (לא 30) - לפי נתון מדויק
    // שנמצא בבדיקה חיצונית (450 קל'/100 גרם × 12.5 גרם = כ-56 קל' ליחידה,
    // לא כ-145 שיצא קודם מה-30 גרם השגויים)
    { name: "עוגיות עבאדי", re: /עבאדי/i, kcal100g: 450, unitGrams: 12.5 },
    ...
    // \s*(?:\d+\s*יחיד(?:ה|ות))?\s* בין "לחמית" לסוג - כדי לתפוס גם ניסוח
    // כמו "לחמית 3 יחידות חיטה מלאה" (כמות בתוך הביטוי, לא רק לפניו/אחריו),
    // לא רק "לחמית חיטה מלאה" צמוד - לפי בקשה מפורשת, אחרי שהתגלה שניסוח
    // כזה נפל בטעות לערך הכללי (16 קל') במקום הספציפי (31 קל')
    { name: "לחמית חיטה מלאה", re: /לחמית\s*(?:\d+\s*יחיד(?:ה|ות))?\s*חיטה מלאה/i, kcal100g: 387, unitGrams: 8 },
    { name: "לחמית כוסמין עם זרעים", re: /לחמית\s*(?:\d+\s*יחיד(?:ה|ות))?\s*כוסמין/i, kcal100g: 411, unitGrams: 9 },
    { name: "לחמית שיפון וקצח", re: /לחמית\s*(?:\d+\s*יחיד(?:ה|ות))?\s*שיפון/i, kcal100g: 375, unitGrams: 8 },
    { name: "לחמית ללא גלוטן", re: /לחמית\s*(?:\d+\s*יחיד(?:ה|ות))?\s*ללא גלוטן/i, kcal100g: 371, unitGrams: 7 },
...
const VOLUME_UNIT_TO_GRAMS = [
    ...
    // כף חומוס (ממרח סמיך) שוקלת משמעותית יותר מ-15 גרם - כמו כף גדושה בשורה
    // שלמעלה, אבל בלי לדרוש "גדושה" מפורשת, כי אפילו כף שטוחה של חומוס סמיך
    // כבר כבדה מכף של נוזל רגיל - לפי בדיקה בפועל מול גוגל (30 קל' שיצא
    // מהאפליקציה מול כ-45-55 קל' האמיתיות, שמתאימות ל-30-40 גרם לא 15)
    { re: /כף חומוס|כפות חומוס|tablespoons? (?:of )?hummus/i, gramsPerUnit: 33 },
...
// עוגיה/עוגייה/עוגיית/עוגיות נוספו כאן - בדיוק כמו פרוסה/פרוסת/פרוסות למטה -
// כדי ש"חצי עוגיית עבאדי" (שם מותג ספציפי שכבר תפוס ע"י ערך משלו במאגר)
// לא ייחשב "לא מכוסה במלואו" רק בגלל מילת-הסיווג הכללית "עוגיית" שנשארת
// לא-מסומנת - מונע הסלמה מיותרת ל-AI במקום להשתמש בנתון המקומי המדויק יותר
// עוגה/עוגת ומאפה/מאפי נוספו כאן באותו טעם בדיוק כמו עוגיה/עוגיית למעלה -
// לפי בקשה מפורשת להרחיב לאותה משפחת מילים (שם-מאכל-כללי בצורת סמיכות
// שמופיע לפני שם מותג/סוג ספציפי שכבר תפוס במאגר)
const FOOD_LEFTOVER_IGNORE_RE = /^(עם|וגם|גם|ו|חצי|רבע|שליש|כפית|כפיות|כף|כפות|כוס|כוסות|גרם|גראם|מ["״]?ל|יחידה|יחידות|קטן|קטנה|גדול|גדולה|בינוני|בינונית|מנה|מנת|פרוסה|פרוסת|פרוסות|עוגיה|עוגייה|עוגיית|עוגיות|עוגה|עוגת|עוגות|מאפה|מאפי|מאפים|של|עוד|קצת|בערך)$/;
